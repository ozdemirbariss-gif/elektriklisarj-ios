...
Ticari yayın için `commercialDataUseApproved` varsayılan kapalıdır. Yalnızca [sağlayıcı koşulları](DATA_PROVIDER_TERMS.md) çözülüp kanıtlandığında açılır; Firebase kurulumunun tamamlanması veri lisansı onayı değildir. Resmi destek adresi `sarjbul@icloud.com`; destek URL'si `Docs/SUPPORT.md` kamuya açık bağlantısıdır.
